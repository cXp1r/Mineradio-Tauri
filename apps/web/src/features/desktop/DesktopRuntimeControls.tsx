...
export function DesktopRuntimeControls(props: DesktopManagementRuntimeResult) {
	...
	return (
		<div className="desktop-runtime-controls" data-busy={props.busy ? "true" : "false"}>
			<div className="fx-section-label">桌面运行时</div>
			<div className="fx-seg" role="group" aria-label="关闭窗口行为">
				<button
					type="button"
			...
			<div className="fx-section-label">缓存治理</div>
			...
			<div className="fx-runtime-cache-list" aria-label="缓存分类使用量">
			...
			<div className="fx-runtime-actions">
				<button type="button" className="fx-mini-btn ghost" disabled={props.busy} onClick={() => void props.refreshCache()}>刷新使用量</button>
				<button type="button" className="fx-mini-btn ghost" disabled={props.busy} onClick={() => void props.chooseCacheRoot()}>选择目录</button>
				<button type="button" className="fx-mini-btn ghost" disabled={props.busy} onClick={() => void props.resetCacheRoot()}>默认目录</button>
			</div>
			<div className="fx-runtime-actions">
				{CACHE_CATEGORIES.map(({ category, label }) => (
					<button
			...
			<div className="fx-section-label">资源治理</div>
			<div className="fx-runtime-actions">
				<button
					type="button"
				...
				>
					{props.workingSetAction.phase === "running" ? "正在整理…" : "整理应用工作集"}
				</button>
			</div>
			<div
			...
			<div className="fx-section-label">Native 诊断</div>
			<div className="fx-runtime-summary">
			...
			<div className="fx-runtime-diagnostic-grid">
				<div><span>生命周期</span><strong>{recordString(lifecycleProbe, "phase") ?? "—"}</strong><small>托盘 {recordString(trayProbe, "trayPhase") ?? "—"}</small></div>
				<div><span>主窗口</span><strong>{windowProbe?.isVisible === true ? "可见" : windowProbe?.isMinimized === true ? "最小化" : "后台"}</strong><small>{windowProbe?.isFocused === true ? "已聚焦" : "未聚焦"}</small></div>
				<div><span>进程内存</span><strong>{workingSetBytes === null ? "—" : formatDesktopBytes(workingSetBytes)}</strong><small>{privateBytes === null ? "private —" : `private ${formatDesktopBytes(privateBytes)}`}</small></div>
				<div><span>Sidecar</span><strong>{recordString(sidecarProbe, "phase") ?? "—"}</strong><small>{recordNumber(sidecarProbe, "pid") === null ? "PID —" : `PID ${recordNumber(sidecarProbe, "pid")}`}</small></div>
				<div><span>桌面歌词</span><strong>{desktopLyricsProbe?.inputWorkerRunning === true ? "输入监听中" : "监听未运行"}</strong><small>{desktopLyricsProbe?.hasPayload === true ? "payload ready" : "等待 payload"}</small></div>
			</div>

			<div className="fx-section-label">Visual 诊断</div>
			<div className="fx-runtime-summary">
			...
			<div className="fx-runtime-diagnostic-grid">
				<div><span>渲染状态</span><strong>{visualDiagnostics?.runtime.mounted ? (visualDiagnostics.runtime.running ? "运行中" : "已挂载") : "未挂载"}</strong><small>{visualDiagnostics ? `${visualDiagnostics.frames.renders} renders` : "—"}</small></div>
				<div><span>帧耗时 P95</span><strong>{visualDiagnostics ? `${visualDiagnostics.frames.frameCostP95Ms.toFixed(1)} ms` : "—"}</strong><small>{visualDiagnostics ? `${visualDiagnostics.frames.longFrames} long frames` : "—"}</small></div>
				<div><span>GPU 资源</span><strong>{visualDiagnostics ? formatDesktopBytes(visualDiagnostics.resources.current.textureBytes + visualDiagnostics.resources.current.geometryBytes) : "—"}</strong><small>{visualDiagnostics ? `${visualDiagnostics.resources.current.meshCount} meshes` : "—"}</small></div>
				<div><span>后台任务</span><strong>{visualDiagnostics ? visualDiagnostics.tasks.queued + visualDiagnostics.tasks.running : "—"}</strong><small>{visualDiagnostics ? `${visualDiagnostics.tasks.failed} failed · ${visualDiagnostics.tasks.staleResultsDropped} stale` : "—"}</small></div>
			</div>
			<div className="fx-runtime-actions">
				<button type="button" className="fx-mini-btn ghost" disabled={props.busy} onClick={() => void props.refreshDiagnostics()}>刷新诊断</button>
			</div>
