...
├── commands.rs       # 8 个 #[tauri::command] + 5 个 reserved window labels
...
capabilities/
└── default.json       # 仅 core:window:default / core:event:default / core:webview:default / core:app:default，scope ["main"]

tauri.conf.json        # 单窗口 main 1280x720，identifier com.mineradio.fork.tauri，CSP null (dev)
Cargo.toml             # deps: tauri 2 / serde / serde_json / dirs 5；GPL-3.0 license
...
| 加新 sidecar capability / plugin | `capabilities/default.json` 在 `permissions` 数组加；如 `shell:allow-open` / `dialog:open` / `fs:read-text-file` —— 当前**仅核心默认**，新 plugin 加后必须 Tauri 权限 + capabilities 双重配置 | 见 `commands.rs::open_external` 用 `std::process::Command` 直 OS 派发以绕过 plugin 缺失 |
...
- **capabilities 最小集**：当前**只 core:window/event/webview/app:default**；新功能（dialog/fs/shell/updater）必须**先加 capability 再创命令**，否则 Tauri `invoke` 返 permission denied
...
- **不要在 capabilities 加 shell/dialog/fs/updater 而不接 plugin**：permission 默认 fail。
...
- **export/import JSON 是 stub**：返回 `Err("EXPORT_IMPORT_NOT_IMPLEMENTED".to_string())`；P10 接 dialog+fs 完成
...
cargo test --manifest-path apps/desktop/src-tauri/Cargo.toml   # 17 pass / 0 fail
