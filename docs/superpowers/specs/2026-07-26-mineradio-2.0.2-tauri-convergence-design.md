...
- `PlaybackAudioRuntime` 已成为唯一 playback Audio owner，独占 A/B deck、pending/committed/retiring owner、Audio Graph、fade、恢复探针、输出路由与 diagnostics；
- application session 只接受 exact issued load handle 与 exact loaded source URL 的 authoritative `ownerchange`；pending/旧 deck 原生事件、stale/forged/duplicate handle 与 source 均不能提交 store；
- 同 Provider/专辑/封面且严格相邻的候选共用 8.5s preload、1.05s muted preroll、360–720ms equal-power handoff、56ms attack 与 180ms adoption slew；loop 队尾到队首、exactly-once advance/adoption、失败和 store rejection rollback 已覆盖；
- play deadline、ready retry、1600/3600ms stall、Graph 与 audibility recovery 均有 generation/lineage 上限；stop/dispose 会释放 physical owner、listener、timer、Graph 与 mirror 资源；
- Visual 只消费只读 `AudioFrameSource` 和数值状态，不创建/持有 playback `HTMLAudioElement`、Controller、AudioContext、MediaElementSource 或 sink mutation；
- `playback.audio.v2` 已承载 fade/gapless/crossfade、primary、最多四个 mirrors 与 Virtual Output Bridge；bridge 归一为 primary，缺失设备保持 typed unavailable 状态，关闭路由恢复系统默认 sink；
- 播放域聚焦 `232 passed`，完整 Bun `2370 passed`，Rust `299 passed`；typecheck、Web build、performance budget、fmt、clippy、freeze audit 与 `git diff --check` 全绿；
- 当前 Bun Sidecar 仍是 `legacy-frozen` production Adapter，开发中的 Rust `MineRadio-api` 未嵌入。M2 不代表单进程/单二进制，也不代表 localhost、supervisor 或 `externalBin` 已移除；
- 正式 WebView2 连播/听感、不同 codec/sample-rate、真实 `setSinkId`/设备拔插/四路 mirror/Virtual Bridge、前后台/睡眠、真实静音误判和 30–60 分钟 Windows soak 为 `Field Validation Pending (non-blocking)`，只阻止 `Field Validated / Release Verified`。
