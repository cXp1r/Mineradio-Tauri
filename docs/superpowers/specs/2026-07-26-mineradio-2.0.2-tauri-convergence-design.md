...
### M4：Lyrics 与 Visual Parity（Open / In Progress）

- Stage Lyrics 2.0、歌词纹理和 GPU 上传预算：`implemented`，clean evidence commit `51ec050`；
- 3D Shelf parity、11/11 对象池与 600×600 soak：`implemented`，clean evidence commit `51ec050`；
- Sonic Topography：技术候选为 `partial`；维护者已基于来源链和公开合作证据作出直接迁移的项目决策，该证据不等于书面授权或许可放宽，当前等待代码复核与新的 release evidence；
- 真实 GPU timer query 已接入 production presentation seam，release strict gate 要求 resolved、non-disjoint 且 `sampleCount > 0`；
- 现有 Bun Sidecar/API/DTO/ProviderId/media URL 与打包行为保持冻结，未接入开发中的 Rust `mineradio-api`。

M4 不得因 Stage/Shelf 已晋升而整体标记 complete；Sonic 直接迁移、origin-attribution 复审与新 evidence 完成前保持 Open / In Progress。
