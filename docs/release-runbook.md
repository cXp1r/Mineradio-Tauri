...
- 选择至少一名独立 reviewer；release actor 和唯一 reviewer 不得是同一人。
...
3. 在 **Deployment protection rules** 中启用 **Required reviewers**：
   - 添加至少一名能够独立核对发布的人员或团队。
   - 启用 **Prevent self-review**。
   - 关闭 **Allow administrators to bypass configured protection rules**。
   - GitHub 通常只要求 reviewer 列表中的任意一人批准；据此选择足够独立的 reviewer 范围。
4. 在 **Deployment branches and tags** 中选择 **Selected branches and tags**。
5. 只新增一条类型为 **Tag**、模式为 `v*` 的规则；不要添加 branch 规则或更宽的 tag 规则。

完成后确认页面同时显示 required reviewers、禁止自审、禁止管理员绕过和唯一的 `v*` tag deployment policy。
...
最后确认两个名称只存在于 `production-release` environment。发布工作流中的 `secrets.TAURI_SIGNING_PRIVATE_KEY` 和 `secrets.TAURI_SIGNING_PRIVATE_KEY_PASSWORD` 只会在 environment 获批后的 job 中解析。
...
- `production-release` environment、reviewer、deployment policy 和 environment secret 已就绪。
...
准备一个严格高于 `v0.1.0` 且四个版本文件完全一致的新版本。指定 release actor 必须验证 tag 精确指向创建时默认分支 tip，并确认该提交中的 workflow 路径是 `protected-release.yml`；随后才创建并推送 tag。签名和发布 job 必须分别停在 `production-release` approval；独立 reviewer 核对 tag、目标提交、版本和 run 来源后再批准。
...
- reviewer、deployment policy、ruleset、immutable releases 或 secret 作用域的任何变更都按发布安全变更审查。
- 每次发布前确认 release actor 与 reviewer 仍独立，且没有新增 bypass actor。
