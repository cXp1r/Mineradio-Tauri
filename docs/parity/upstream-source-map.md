...
| Sonic Workshop | 2.0.3 `public/sonic-workshop-preset.js`、`public/vendor/sonic-workshop/**`；CmzYa / Workshop `3747222633` | migration-pending，尚未实现 | `packages/visual-engine/src/sonic-workshop` (future) | 已决定依据可观察行为独立重实现；不得用 Sonic Topography/Ajin provenance 代替，不复制或再分发 vendor bundle，legacy numeric `8` 继续迁为 `7`；边界与预算见 `sonic-workshop-module-design.md` |
