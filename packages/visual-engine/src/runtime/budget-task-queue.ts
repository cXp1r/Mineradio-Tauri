...
export function createBudgetTaskQueue(options: BudgetTaskQueueOptions): BudgetTaskQueue {
	const cancellationScope = options.cancellationScope ?? createCancellationScope("budget-task-queue");
	...
	const isCommitAllowed = (entry: QueueEntry) =>
		!disposed &&
	...
	return {
		enqueue(task) {
			if (disposed || !options.resourceScope.isOpen() || !cancellationScope.isOpen()) return false;
		...
		runSlice(costBudget) {
			...
			if (disposed || !options.resourceScope.isOpen()) return 0;
			cancelLowPriorityForHardPressure();
			let spent = 0;
			let started = 0;
			while (true) {
		...
		dispose() {
			if (disposed) return;
			disposed = true;
			for (const entry of [...queued, ...activeById.values()]) cancelEntry(entry);
