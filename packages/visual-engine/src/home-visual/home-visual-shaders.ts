...
export const HOME_VISUAL_FRAGMENT_SHADER = `precision highp float;
...
export const HOME_VISUAL_BLOOM_FRAGMENT_SHADER = `precision highp float;
uniform sampler2D uDotTex;
...
export function buildHomeVisualBloomVertexShader(vs: string): string {
	return vs
		.replace(
			"uniform float uMouseActive, uPixel, uColorMixT, uLoading;",
